...
def plotdecodeimages():
    ...
    # guessed_z = model.network['features']
    #FLAGS.model == 'UNet'
    ...
    batch_count = 0
    if FLAGS.encoder_type == 'Video':
        dataset_list_features = np.zeros([data_size, 1024], dtype=float)
    elif FLAGS.encoder_type == 'Audio':
        dataset_list_features = np.zeros([data_size, 256], dtype=float)
    elif FLAGS.encoder_type == 'Ac':
        dataset_list_features = np.zeros([data_size, 150], dtype=float)
    else:
        dataset_list_features = np.zeros([data_size, 4, 4, 8], dtype=float)
