...
class Trainer(object):
    ...
    def _build_functions(self, data):
        ...
        guessed_z_ac = self.modelac.mean + (self.modelac.variance * samples)
        if FLAGS.fusion:
            mean = (self.modelimages.mean + self.modelaudio.mean)/2
            var = (self.modelaudio.variance + self.modelimages.variance)/2
            z = mean + (var * samples)
            self.tripletloss, _ = self.mix_all(guessed_z_ac, z, self.labels, self.scenario,
                                                        FLAGS.margin)
        elif FLAGS.moddrop:
            meanvideo, variancevideo, onvideo = self.modDrop(self.modelimages.mean, self.modelimages.variance, self.modelimages.network['is_training'], p_mod=0.98)
            meanaudio, varianceaudio, onaudio = self.modDrop(self.modelaudio.mean, self.modelaudio.variance, self.modelaudio.network['is_training'], p_mod=0.98)
            meanac, varianceac, onac = self.modDrop(self.modelac.mean, self.modelac.variance, self.modelac.network['is_training'])
            modalities = onac + onaudio + onvideo
            modalities = tf.squeeze(modalities)
            modalities = tf.cond(modalities > 0, lambda: modalities, lambda: modalities + 1e-15)
            mean = (meanac + meanaudio + meanvideo)/modalities
            var = (varianceac + varianceaudio + variancevideo)/modalities
            z = mean + (var * samples)
            self.tripletloss, _ = self.mix_all(guessed_z_ac, z, self.labels, self.scenario,
                                                        FLAGS.margin)
        elif FLAGS.l2:
            meanac, varianceac = self.modelac.mean, self.modelac.variance
            meanaudio, varianceaudio = self.modelaudio.mean, self.modelaudio.variance
            meanvideo, variancevideo = self.modelimages.mean, self.modelimages.variance
            ...
            self.l2variancevideoac = tf.losses.mean_squared_error(variancevideo, varianceac)
            self.l2varianceaudioac = tf.losses.mean_squared_error(varianceaudio, varianceac)
        else:
            guessed_z_audio = self.modelaudio.mean + (self.modelaudio.variance * samples)
            guessed_z_video = self.modelimages.mean + (self.modelimages.variance * samples)
            self.tripletloss_video_ac, _ = self.mix_data_hard(guessed_z_ac, guessed_z_video, self.labels, self.scenario,
                                                        FLAGS.margin)
            self.tripletloss_audio_ac, _ = self.mix_data_hard(guessed_z_ac, guessed_z_audio, self.labels, self.scenario,
                                                        FLAGS.margin)
            self.tripletloss = self.tripletloss_video_ac + self.tripletloss_audio_ac
    ...
    def train(self, train_data=None, valid_data=None):
        ...
        if FLAGS.l2:
            self.logger.log_scalar('l2meanvideoac', self.l2meanvideoac)
            self.logger.log_scalar('l2variancevideoac', self.l2variancevideoac)
            self.logger.log_scalar('l2meanvaudioac', self.l2meanaudioac)
            self.logger.log_scalar('l2varianceaudioac', self.l2varianceaudioac)
        else:
            self.logger.log_scalar('triplet_loss', self.tripletloss)
            if not FLAGS.fusion and not FLAGS.moddrop:
                self.logger.log_scalar('triplet_loss_video_ac', self.tripletloss_video_ac)
                self.logger.log_scalar('triplet_loss_audio_ac', self.tripletloss_audio_ac)
    ...
    def modDrop(self, mean, variance, is_training, p_mod=.5):
        on = tf.cast(tf.random_uniform([1]) - p_mod < 0, tf.float32)
        mean = tf.cond(is_training, lambda: on * mean, lambda: mean)
        variance = tf.cond(is_training, lambda: on * variance, lambda: variance)
        return mean, variance, on
