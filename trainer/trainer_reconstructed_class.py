...
class Trainer(object):
    ...
    def __init__(self, model, model_encoder_acoustic, model_encoder_images,  display_freq=1,
                 learning_rate=0.0001, num_classes=14, num_epochs=1, nr_frames=12, temporal_pooling=False):

        self.model = model
        self.model_encoder_acoustic = model_encoder_acoustic
        self.model_encoder_images = model_encoder_images
    ...
    def _build_functions(self, data):
        ...
        self.model_encoder_acoustic._build_model(mfccmap, self.model_encoder_images.output)
        output = tf.reshape(self.model_encoder_acoustic.output, shape=[-1, 12, 36, 48, 12])
    ...
    def _restore_model(self, session):
        ...
        var_list = var_list2 + var_list1 + var_list_ac
        # else:
        #     var_list = slim.get_model_variables(self.model.scope)
        saver = tf.train.Saver(var_list=var_list)
                    ...
                    try:
                        ...
                        train_loss, train_accuracy, train_summary, _ = session.run(
                            ...
                            feed_dict={self.handle: train_handle,
                                       ...
                                       self.model_encoder_images.network['keep_prob']: 1.0,
                                       self.model_encoder_images.network['is_training']: 0,
                                       self.model_encoder_acoustic.network['keep_prob']: 1.0,
                                       self.model_encoder_acoustic.network['is_training']: 0
                                       })
