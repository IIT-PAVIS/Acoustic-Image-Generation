...
class Trainer(object):
    ...
    def __init__(self, modelac, modelaudio, modelimages, modelassociator, modelassociator1, logger=None, display_freq=1,
    ...
    def _build_functions(self, data):
        ...
        #find feature maps of 3
        output = self.modelac._build_network(self.acoustic)
        if FLAGS.moddrop:
            output = self.modDrop(output, is_training=self.modelac.is_training, p_mod=0.2)
        outputvideo = self.modelimages._build_network(self.video)
        outputaudio = self.modelaudio._build_network(self.mfcc)
        #fuse feature maps and get new feature maps for 3 mod
        if FLAGS.fusion:
            self.modelassociator._build_model(outputvideo, outputaudio)
        else:#if all or onlyaudiovideo
            self.modelassociator._build_model(output, outputvideo, outputaudio)

        if not FLAGS.onlyaudiovideo:
            self.modelac._build_model(self.modelassociator.outputac)
            self.modelaudio._build_model(self.modelassociator.outputaudio)
            self.modelimages._build_model(self.modelassociator.outputvideo)
            #use only reconstruction losses
            ...
            self.latent_lossaudio = 0.5 * tf.reduce_sum(
                tf.square(self.modelaudio.mean) + tf.square(self.modelaudio.std)
                - tf.log(1e-8 + tf.square(self.modelaudio.std)) - 1, 1)
            self.latent_lossvideo = 0.5 * tf.reduce_sum(
                tf.square(self.modelimages.mean) + tf.square(self.modelimages.std)
            ...
            with tf.control_dependencies(update_ops):
                with tf.device('/gpu:0'):
                    self.train_op_0 = self.optimizer.minimize(loss=self.loss,
                                                               var_list=self.modelassociator.train_vars,
                                                               global_step=self.global_step)
        else:# onlyaudiovideo
            #build new associator
            self.modelassociator1._build_model(outputvideo, outputaudio)
            self.modelac._build_model(self.modelassociator1.outputac)
            self.l2_feature = tf.losses.mean_squared_error(self.modelassociator.outputac, self.modelassociator1.outputac)
            self.losslmse = tf.losses.mean_squared_error(self.acoustic, self.modelac.output)
            self.l1 = tf.losses.huber_loss(self.acoustic, self.modelac.output)
            self.latent_lossac = 0.5 * tf.reduce_sum(tf.square(self.modelac.mean) + tf.square(self.modelac.std)
                                                     - tf.log(1e-8 + tf.square(self.modelac.std)) - 1, 1)
            self.latent_loss = tf.reduce_mean(self.latent_lossac, 0) / 1000000
            self.loss = self.latent_loss + tf.losses.get_total_loss()
            # slim.get_model_variables(self.scope + '/logits')
            # Initialize counters and stats
            self.global_step = tf.train.create_global_step()
            ...
            with tf.control_dependencies(update_ops):
                with tf.device('/gpu:0'):
                    ...
                    self.train_op_0 = self.optimizer.minimize(loss=self.loss,
                                                              var_list=self.modelassociator1.train_vars,
                                                              global_step=self.global_step)
    ...
    def _init_model(self, session):
        ...
        if FLAGS.init_checkpoint is not None:
            ...
            var_list_ac = slim.get_variables(self.modelac.scope)
            var_list_audio = slim.get_variables(self.modelaudio.scope)
            var_list_video = slim.get_variables(self.modelimages.scope + '/')
            var_list_associator = slim.get_variables(self.modelassociator.scope + '/')
            # variables_in_checkpoint = tf.train.list_variables(FLAGS.acoustic_init_checkpoint)
            var_list = var_list_ac + var_list_audio + var_list_video + var_list_associator
    ...
    def train(self, train_data=None, valid_data=None):
        ...
        if FLAGS.onlyaudiovideo:
            self.logger.log_scalar('l2_feature', self.l2_feature)
            ...
            for epoch in range(start_epoch, start_epoch + self.num_epochs):
                ...
                while True:
                    try:
                        ...
                        latentloss, train_loss, train_summary, _= session.run([self.latent_loss,
                            self.loss, self.logger.summary_op, self.train_op_0],  # self.train_op_1
                                    feed_dict={self.handle: train_handle,
                                       self.modelac.keep_prob: 0.5,
                                       self.modelac.is_training: 1,
                                       self.modelaudio.keep_prob: 0.5,
                                       self.modelaudio.is_training: 1,
                                       self.modelimages.keep_prob: 0.5,
                                       self.modelimages.is_training: 1})

                        # Compute mini-batch error
                        if step % self.display_freq == 0:
                            print('{}: {} - Iteration: [{:3}]\t Training_Loss:'
                                  ' {:6f}\t Latent_Loss: {:6f} '.format(
                                    datetime.now(), FLAGS.exp_name, step, train_loss, latentloss))
            ...
            try:
                ...
                one_hot_labels, batch_loss = session.run(
                    ...
                    feed_dict={self.handle: eval_handle,
                               self.modelac.keep_prob: 1.0,
                               self.modelac.is_training: 0,
                               self.modelaudio.keep_prob: 1.0,
                               self.modelaudio.is_training: 0,
                               self.modelimages.keep_prob: 1.0,
                               self.modelimages.is_training: 0})
                # Update counters
    ...
    def modDrop(self, f, is_training, p_mod=.5):
