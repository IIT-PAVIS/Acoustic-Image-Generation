...
def plotdecodeimages():
    ...
    dataset_list_featuresac = np.zeros([data_size, 9, 12, num_embedding], dtype=float)
    dataset_list_featuresaudio = np.zeros([data_size, 6, 16, num_embedding], dtype=float)
    dataset_list_featuresimages = np.zeros([data_size, 14, 18, num_embedding], dtype=float)
        ...
        while True:
            try:
                 labels_data, scenario_data, featuresac, featuresaudio, featuresimages = session.run(
                    [labels, scenario, extractedac, extractedaudio, extractedvideo],
