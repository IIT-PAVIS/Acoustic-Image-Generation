...
class ActionsDataLoader(object):
    ...
    def __init__(self, txt_file, mode, batch_size, sample_rate=22050, sample_length=2, embedding=1,
                 buffer_size=1, num_epochs=1, shuffle=False, normalize=False, correspondence=0,
                 random_pick=False, build_spectrogram=False, modalities=None, nr_frames=12, datakind='old'):
    ...
    def _map_function(self, audio_images, audio_samples, video_images, action, location, subject):
        ...
        subject_encoded = tf.one_hot(
            tf.squeeze(tf.gather(subject, tf.range(self.sample_length, delta=self.sample_length))), self.num_subjects)
    ...
    def _map_func_video_images_pick_all_frames(self, audio_images, audio_samples, video_images, action, location, subject):
        """Pick nr_frames random frames."""
        # in 1 second 12
        selected_video_images = video_images
        selected_audio_samples = audio_samples
        selected_audio_images = audio_images
        if self.embedding:
            ...
            subject = tf.tile(subject, (1, 12))
            subject = tf.reshape(subject, (-1, self.num_subjects))
        return selected_audio_images, selected_audio_samples, selected_video_images, action, location, subject
    ...
    def _map_func_video_images_pick_frames(self, audio_images, audio_samples, video_images, action, location, subject):
        ...
        selected_video_images, selected_audio_samples, selected_audio_images = \
            self._pick_random_frames(video_images, audio_samples, audio_images)
        action = tf.expand_dims(action, 0)
        ...
        location = tf.reshape(location, (-1, self.num_locations))
        subject = tf.expand_dims(subject, 0)
        subject = tf.tile(subject, (1, 1))
        subject = tf.reshape(subject, (-1, self.num_subjects))
        return selected_audio_images, selected_audio_samples, selected_video_images, action, location, subject

    def _pick_random_frames(self, samples, audio_samples, audio_images):
        ...
        return frames, framesaudio, framesaudioimage
    ...
    def _map_func_acoustic_images(self, audio_images, audio_samples, video_images, action, location, subject):
        """Input mapper to pre-processes the given image for training."""
        ...
        return processed_images, audio_samples, video_images, action, location, subject
    ...
    def _map_func_mfcc(self, audio_images, audio_samples, video_images, action, location, subject):
        """Input mapper to pre-processes the given image for training."""
    ...
    def _map_func_audio_samples_build_spectrogram(self, audio_images, audio_wav, processed_images, action, location, subject):
        """Input mapper to build waveform audio from raw audio samples."""

        magnitude_spectrograms = tf.py_func(self._build_spectrograms_function, [audio_wav], tf.float32)
        magnitude_spectrograms = tf.reshape(magnitude_spectrograms,
                                            (self.sample_length * _FRAMES_PER_SECOND, self.num_channels))
    ...
    def _map_func_correspondence(self, audio_images, magnitude_spectrograms, processed_images, action, location, subject):

        action = tf.argmax(action, axis=1)
        location = tf.argmax(location, axis=1)
        subject = tf.argmax(subject, axis=1)
        # find  shuffled class, video and images
        location2 = tf.random_shuffle(location, seed=2)
        action2 = tf.random_shuffle(action, seed=2)
        subject2 = tf.random_shuffle(subject, seed=2)
        ...
        videosubject = tf.concat((subject, subject), 0)
        ...
        audiosubject = tf.one_hot(audiosubject, self.num_subjects)
        videoaction = tf.one_hot(videoaction, self.num_actions)
        videolocation = tf.one_hot(videolocation, self.num_locations)
        videosubject = tf.one_hot(videosubject, self.num_subjects)
        ...
        return audio_images, reshaped_audio_samples, reshaped_video_images, videoaction, \
               videolocation, audioaction, audiolocation, labels, videosubject, audiosubject
