...
class ActionsDataLoader(object):
    ...
    def __init__(self, txt_file, mode, batch_size, sample_rate=22050, sample_length=2,
    ...
    def _load_acoustic_images_stats(self):
        ...
        threshold = tf.constant(
            ...
            dtype=tf.float32,
            shape=[_FRAMES_PER_SECOND * self.sample_length, 36, 48, self.num_channels]
        )
    ...
    def _map_func_video_images_pick_frames(self, audio_images, audio_samples, video_images, action, location):
        ...
        # selected_audio_images = self._pick_mean(audio_images)
        return audio_images, audio_samples, selected_video_images, action, location

    def _pick_random_frames(self, samples):
    ...
    def _pick_mean(self, samples):
        ...
        # pick nr_frames
        frames = self._sample_mean(samples, num_frames, n_to_sample)
        return frames

    def _sample_mean(self, samples, num_frames, sample_size):
    ...
    # def _map_func_audio_samples_build_wav2(self, audio_images, audio_samples, video_images, action, location):
    ...
    def _map_func_video_images(self, audio_images, audio_samples, video_images, action, location):
        ...
        return audio_images, audio_samples, processed_images, action, location
    ...
    def _map_func_acoustic_images(self, audio_images, audio_samples, video_images, action, location):
        """Input mapper to pre-processes the given image for training."""
        ...
        return processed_images, audio_samples, video_images, action, location
    ...
    #     magnitude_spectrograms = tf.reshape(magnitude_spectrograms, (self.sample_length * _FRAMES_PER_SECOND, 1, 1, 13))
    ...
    #         dct_base[:, m] = np.cos((m) * np.pi / filter_num * (np.arange(filter_num) + 0.5))
    #     lifter = 1 + (lifter_num / 2) * np.sin(np.pi * (np.arange(mfcc_num)) / lifter_num)
    #
    ...
    def _map_func_correspondence(self, audio_images, magnitude_spectrograms, processed_images, action, location):
        ...
        # find  shuffled class, video and images
        location2 = tf.random_shuffle(location, seed=2)
        action2 = tf.random_shuffle(action, seed=2)
        processed_images2 = tf.random_shuffle(processed_images, seed=2)
        audio_images2 = tf.random_shuffle(audio_images, seed=2)
        magnitude_spectrograms2 = tf.random_shuffle(magnitude_spectrograms, seed=2)
        ...
        reshaped_audio_samples = tf.concat((magnitude_spectrograms, magnitude_spectrograms2), 0)
        audiolocation = tf.concat((location, location2), 0)
        audioaction = tf.concat((action, action2), 0)


        # correspondence labels
        labels_equal = tf.equal(action, action2)
        scenario_equal = tf.equal(location, location2)
        labels2 = tf.logical_and(scenario_equal, labels_equal)
        labels2 = tf.cast(labels2, dtype=tf.int32)
        # first part ones second zero a part where it is picking same clip
        labels = tf.concat((tf.ones((tf.shape(processed_images2)[0]), dtype=tf.int32),
                            labels2), 0)
        # one hot encoding
        labels = tf.one_hot(labels, 2)
        audioaction = tf.one_hot(audioaction, self.num_actions)
        audiolocation = tf.one_hot(audiolocation, self.num_locations)
        ...
        return audio_images, reshaped_audio_samples, reshaped_video_images, videoaction, \
               videolocation, audioaction, audiolocation, labels
