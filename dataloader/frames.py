...
class ActionsDataLoader(object):
    ...
    def __init__(self, txt_file, mode, batch_size, sample_rate=12288, sample_length=2, embedding=1,
                 ...
                 random_pick=False, build_spectrogram=False, modalities=None, nr_frames=1, datakind='frames'):
        ...
        # one frames from video sequence
        # if random_pick:
    ...
    def _parse_sequence(self, sequence_example_proto):
        ...
        context_features = {}
        sequence_features = {'xmin': tf.FixedLenSequenceFeature([], dtype=tf.string),
                            'xmax': tf.FixedLenSequenceFeature([], dtype=tf.string),
                            'ymin': tf.FixedLenSequenceFeature([], dtype=tf.string),
                            'ymax': tf.FixedLenSequenceFeature([], dtype=tf.string),
                            'typescene': tf.FixedLenSequenceFeature([], dtype=tf.string)}
        ...
        xmin = tf.decode_raw(parsed_sequence_features['xmin'], tf.int32)
        xmin = tf.reshape(xmin, [-1, 3])
        xmax = tf.decode_raw(parsed_sequence_features['xmax'], tf.int32)
        xmax = tf.reshape(xmax, [-1, 3])
        ymin = tf.decode_raw(parsed_sequence_features['ymin'], tf.int32)
        ymin = tf.reshape(ymin, [-1, 3])
        ymax = tf.decode_raw(parsed_sequence_features['ymax'], tf.int32)
        ymax = tf.reshape(ymax, [-1, 3])
        typescene = tf.decode_raw(parsed_sequence_features['typescene'], tf.int32)
        typescene = tf.reshape(typescene, [-1, 3])
        ...
        if self.include_audio_images:
            ...
            audio_images = tf.zeros([1, 36, 48, self.num_channels], tf.int32)
        ...
        return audio_images, audio_samples, video_images, xmin, xmax, ymin, ymax, typescene, num_samples

    def _map_function(self, audio_images, audio_samples, video_images, xmin, xmax, ymin, ymax, typescene, num_samples):
        ...
        if self.include_audio_data:
            ...
            reshaped_audio_samples = tf.reshape(audio_samples, [-1, 1, num_samples])
        else:
        ...
        return reshaped_audio_images, reshaped_audio_samples, reshaped_video_images, xmin, xmax, ymin, ymax, typescene
    ...
    def _map_func_audio_samples_build_wav(self, audio_images, audio_samples, video_images, xmin, xmax, ymin, ymax, typescene):
        """Input mapper to build waveform audio from raw audio samples."""

        filtered_wav = tf.py_func(self.butter_lowpass_filter, [audio_samples], tf.float32)
        return audio_images, audio_samples, video_images, xmin, xmax, ymin, ymax, typescene, filtered_wav
    ...
    def _map_func_video_images(self, audio_images, audio_samples, video_images, xmin, xmax, ymin, ymax, typescene, filtered_audio_samples):
        ...
        return audio_images, audio_samples, processed_images, xmin, xmax, ymin, ymax, typescene, filtered_audio_samples
    ...
    def _map_func_acoustic_images(self, audio_images, audio_samples, video_images, xmin, xmax, ymin, ymax, typescene, filtered_audio_samples):
        """Input mapper to pre-processes the given image for training."""
        ...
        return processed_images, audio_samples, video_images, xmin, xmax, ymin, ymax, typescene, filtered_audio_samples
    ...
    def _map_func_mfcc(self, audio_images, audio_samples, video_images, xmin, xmax, ymin, ymax, typescene, filtered_audio_samples):
        """Input mapper to pre-processes the given image for training."""
        ...
        return audio_images, audio_samples, video_images, xmin, xmax, ymin, ymax, typescene, filtered_audio_samples
    ...
    def _map_func_audio_samples_build_spectrogram(self, audio_images, audio_wav, processed_images, xmin, xmax, ymin, ymax, typescene, filtered_audio_wav):
        """Input mapper to build waveform audio from raw audio samples."""

        magnitude_spectrograms = tf.py_func(self._build_spectrograms_function, [audio_wav], tf.float32)
        magnitude_spectrograms = tf.reshape(magnitude_spectrograms, (self.sample_length, self.num_channels))

        filtered_magnitude_spectrograms = tf.py_func(self._build_spectrograms_function, [filtered_audio_wav], tf.float32)
        filtered_magnitude_spectrograms = tf.reshape(filtered_magnitude_spectrograms, (self.sample_length, self.num_channels))

        return audio_images, magnitude_spectrograms, processed_images, xmin, xmax, ymin, ymax, typescene, filtered_magnitude_spectrograms

    def _build_spectrograms_function(self, audio_data):

        # n = np.shape(audio_data)[0]
        length = np.shape(audio_data)[1]
        # window = signal.tukey(length, alpha=0.75)
        # window = np.tile(window, (n, 1))
        # window = np.reshape(window, (n, _NUMBER_OF_SAMPLES))
        # raw_audio = audio_data * window
        fftdata = np.abs(np.fft.rfft(audio_data, length, axis=1))[:, :-1]
        fftdata = fftdata ** 2
    ...
    def _map_func_audio_samples_build_spectrogram2(self, audio_images, audio_wav, processed_images, xmin, xmax, ymin, ymax, typescene):
        ...
        return audio_images, magnitude_spectrograms, processed_images, xmin, xmax, ymin, ymax, typescene

    def _map_func_correspondence(self, audio_images, magnitude_spectrograms, processed_images, xmin, xmax, ymin, ymax, typescene, filtered_magnitude_spectrograms):
        ...
        return audio_images, reshaped_audio_samples, reshaped_video_images, xmin, xmax, ymin, ymax, typescene
