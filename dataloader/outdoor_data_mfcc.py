...
class ActionsDataLoader(object):
    ...
    def __init__(self, txt_file, mode, batch_size, sample_rate=12288, sample_length=2, embedding=1,
                 buffer_size=1, num_epochs=1, shuffle=False, normalize=False, correspondence=0, correspondencevideo=0,
                 random_pick=False, build_spectrogram=False, modalities=None, nr_frames=12, datakind='outdoor'):
        ...
        self.correspondencevideo = correspondencevideo
        if self.datakind == 'outdoor':
            self.num_channels = 12
            self.num_actions = 10
            self.num_locations = 61  # maximum number of videos for a class
        ...
        if self.datakind == 'music':
            data = data.map(self._parse_sequence_music, num_parallel_calls=4)
        else:
        ...
        if self.correspondence:
            ...
            if mode == 'training':
                ...
                data = data.shuffle(buffer_size=buffer_size, reshuffle_each_iteration=True)
                data = data.batch(batch_size)
        if self.correspondencevideo:
            data = data.map(self._map_func_correspondence_no_video, num_parallel_calls=4)
    ...
    def _parse_sequence(self, sequence_example_proto):
        ...
        return audio_images, audio_samples, video_images, action, location

    def _parse_sequence_music(self, sequence_example_proto):
        """Input parser for samples of the training set."""
        ...
        if self.include_audio_images:
            ...
            audio_image_shape = tf.stack([-1, audio_height, audio_width, audio_depth])
            audio_images = tf.reshape(audio_image_decoded, audio_image_shape)
            #audio_images = tf.image.random_flip_left_right(audio_images, seed=3)
            # audio_images = tf.image.random_flip_up_down(audio_images, seed=2)
        else:
            audio_images = tf.zeros([12, 36, 48, self.num_channels], tf.int32)
        ...
        if self.include_video_data:
            ...
            video_image_shape = tf.stack([-1, video_height, video_width, video_depth])  # 224, 298, 3
            video_images = tf.reshape(video_image_decoded, video_image_shape)
            #video_images = tf.image.random_flip_left_right(video_images, seed=3)
            # video_images = tf.image.random_flip_up_down(video_images, seed=2)
        else:
            video_images = tf.zeros([], tf.int32)

        return audio_images, audio_samples, video_images, action, location

    def _map_function(self, audio_images, audio_samples, video_images, action, location):
        ...
        return reshaped_audio_images, reshaped_audio_samples, reshaped_video_images, action_encoded, location_encoded

    def _map_func_video_images_pick_all_frames(self, audio_images, audio_samples, video_images, action, location, filtered_audio_samples):
        """Pick nr_frames random frames."""
        #in 1 second 12
        if self.embedding:
            action = tf.expand_dims(action, 0)
            action = tf.tile(action, (1, 12))
            action = tf.reshape(action, (-1, self.num_actions))
            location = tf.expand_dims(location, 0)
            location = tf.tile(location, (1, 12))
            location = tf.reshape(location, (-1, self.num_locations))
        return audio_images, audio_samples, video_images, action, location, filtered_audio_samples

    def _map_func_video_images_pick_frames(self, audio_images, audio_samples, video_images, action, location, filtered_audio_samples):
        """Pick nr_frames random frames."""
        # in 1 second 12
        ...
        action = tf.expand_dims(action, 0)
        action = tf.tile(action, (1, 1))
        action = tf.reshape(action, (-1, self.num_actions))
        location = tf.expand_dims(location, 0)
        location = tf.tile(location, (1, 1))
        location = tf.reshape(location, (-1, self.num_locations))
        return selected_audio_images, selected_audio_samples, selected_video_images, action, location, filtered_selected_audio_samples

    def _pick_random_frames(self, samples, audio_samples, filtered_audio_samples, audio_images):
        num_frames = tf.shape(samples)[0]  # how many images
        n_to_sample = tf.constant([1])  # how many to keep, 5 in temporal resnet 50, 8 in resnet18
        ...
        return frames, framesaudio, filtered_framesaudio, framesaudioimage
    ...
    def _map_func_audio_samples_build_wav(self, audio_images, audio_samples, video_images, action, location):
        ...
        return audio_images, audio_wav, video_images, action, location, filtered_wav
    ...
    def _map_func_audio_images_piece_wise(self, audio_images, audio_samples, video_images, action, location):
        ...
        return audio_images_norm, audio_samples, video_images, action, location

    def _map_func_audio_images_min_max_norm(self, audio_images, audio_samples, video_images, action, location):
    ...
    def _map_func_audio_samples_min_max_norm(self, audio_images, audio_samples, video_images, action, location):
        ...
        return audio_images, audio_samples_norm, video_images, action, location

    def _map_func_audio_samples_mean_norm(self, audio_images, audio_samples, video_images, action, location):
        """Input mapper to apply min-max normalization to the audio samples."""
    ...
    def _map_func_video_images(self, audio_images, audio_samples, video_images, action, location, filtered_audio_samples):
        ...
        return audio_images, audio_samples, processed_images, action, location, filtered_audio_samples
    ...
    def _map_func_acoustic_images(self, audio_images, audio_samples, video_images, action, location, filtered_audio_samples):
        """Input mapper to pre-processes the given image for training."""
        ...
        return processed_images, audio_samples, video_images, action, location, filtered_audio_samples
    ...
    def _map_func_mfcc(self, audio_images, audio_samples, video_images, action, location, filtered_audio_samples):
        """Input mapper to pre-processes the given image for training."""
    ...
    def _map_func_audio_samples_build_spectrogram(self, audio_images, audio_wav, processed_images, action, location, filtered_audio_wav):
        """Input mapper to build waveform audio from raw audio samples."""
        ...
        return audio_images, magnitude_spectrograms, processed_images, action, location, filtered_magnitude_spectrograms
    ...
    def _map_func_audio_samples_build_spectrogram2(self, audio_images, audio_wav, processed_images, action, location):
        ...
        return audio_images, magnitude_spectrograms, processed_images, action, location

    def _map_func_correspondence(self, audio_images, magnitude_spectrograms, processed_images, action, location,
                                 filtered_magnitude_spectrograms):
        ...
        # mfcc = filtered_magnitude_spectrograms - tf.reduce_min(filtered_magnitude_spectrograms, axis=[1],
        #                                                        keep_dims=True)
        # mfcc = mfcc / tf.reduce_max(mfcc, axis=[1], keep_dims=True)
        mfccmap = tf.reshape(filtered_magnitude_spectrograms, (-1, 1, 12))
        ...
        return audio_images, reshaped_audio_samples, reshaped_video_images, videoaction, \
               videolocation, labels

    def _map_func_correspondence_no_video(self, audio_images, magnitude_spectrograms, processed_images, action, location,
                                 filtered_magnitude_spectrograms):
        # for each data we have copied silence mfcc, we tile to obtain corresponding acoustic images and double data
        action = tf.argmax(action, axis=1)
        location = tf.argmax(location, axis=1)
        ...
        mfccmap = tf.reshape(mfccmap, (-1, 36, 48, 12))
        audio_images2 = mfccmap
        zeros_processed_images = tf.zeros_like(processed_images)

        # concatenate two times
        reshaped_video_images = tf.concat((processed_images, zeros_processed_images), 0)
        videoaction = tf.concat((action, action), 0)
        videolocation = tf.concat((location, location), 0)
        audio_images = tf.concat((audio_images, audio_images), 0)
        reshaped_audio_samples = tf.concat((magnitude_spectrograms, magnitude_spectrograms), 0)
