...
flags.DEFINE_integer('batch_size', 1, 'Batch size choose')
...
'''Create plots of background noise and sound'''
...
def plotdecodeimages():
    ...
    name = '{}_{}_{}_{}'.format(FLAGS.model, dataset, 'Couple_background_noise', name)
    ...
    with tf.device('/cpu:0'):
        train_data = ActionsDataLoader(FLAGS.train_file, 'testing', batch_size=FLAGS.batch_size, num_epochs=1, sample_length=1,
                                      datakind='outdoor', buffer_size=10, shuffle=False,
                                       normalize=normalize, build_spectrogram=build_spectrogram, correspondence=1,
                                       random_pick=random_pick, modalities=modalities, nr_frames=nr_frames)
    ...
    if FLAGS.datatype == 'music':
        ...
        num_actions = 10
        num_locations = 61
    labels_video = tf.reshape(next_batch[3], shape=[-1, num_actions])
    labels_video = tf.argmax(labels_video, axis=1)
    scenario_video = tf.reshape(next_batch[4], shape=[-1, num_locations])
    scenario_video = tf.argmax(scenario_video, axis=1)
    # labels_audio = tf.reshape(next_batch[5], shape=[-1, num_actions])
    # labels_audio = tf.argmax(labels_audio, axis=1)
    # scenario_audio = tf.reshape(next_batch[6], shape=[-1, num_locations])
    # scenario_audio = tf.argmax(scenario_audio, axis=1)
    lab = tf.reshape(next_batch[5], shape=[-1, 2])
    lab = tf.argmax(lab, axis=1)
    ...
    namesimage2 = ['Acoustic image', 'Rec 1', 'Rec 2']
    ...
    with tf.Session(
        ...
        while True:
            try:
                data, reconstructed, im, labcorr, labv, scev = session.run(
                    [acoustic, output, images, lab, labels_video, scenario_video],
                ...
                #reorder images in an array according to number of classes
                num_seen_cl = len(np.unique(labv))
                #columns ac correct audio wrong audio rows video
                matrixac = np.zeros((3, 36, 48, 12), dtype=np.float32)
                #rows video
                matrixvideo = np.zeros([224, 298, 3], dtype=np.float32)
                for h in range(np.shape(reconstructed)[0]):
                    #if correspondent first two columns of correct ac and audio and fill video
                    if labcorr[h]:
                        #if correspondent save video
                        matrixvideo = im[h]
                        matrixac[0] = data[h]
                        matrixac[1] = reconstructed[h]
                    else:
                        #last column not correspondent reconstructed acoustic image from different video
                        matrixac[2] = reconstructed[h]
                        # matrixac[h%2, 3] = data[h]
                ...
                for j in range(3):
                    # x = 0
                    y = j
                    imgray = cv2.cvtColor(matrixvideo, cv2.COLOR_BGR2GRAY)
                    axs[y].imshow(imgray, cmap=plt.cm.gray)
                    map = find_logen(matrixac[j])
                    map = cv2.resize(map, (298, 224))
                    axs[y].imshow(map, cmap=plt.cm.jet, alpha=0.7)
                    axs[y].axis('off')
                    axs[y].set_title('{}'.format(namesimage2[j]), fontsize=10)
                outImage_path = '{}/{}_imagesall_{}.png'.format(data_dir, dataset, num2)
                ...
                #     plt.tight_layout(pad=1.0)
                #     fig.suptitle('audio {} video {} corr{}'.format(classes[laba[h]], classes[labv[h]], labcorr[h]))
                ...
                #         map = cv2.resize(map, (298, 224))
                #         axs[y].imshow(map, cmap=plt.cm.viridis, alpha=0.7)
                #         axs[y].axis('off')
