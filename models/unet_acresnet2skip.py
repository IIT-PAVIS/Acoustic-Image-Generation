...
class UNetAc(object):
    ...
    def _build_network(self, inputs, resnetfeature, is_training=True, keep_prob=0.5, weight_decay=1e-6, scope='UNetAcRes'):
        ...
        with tf.variable_scope(scope, 'UNetAcRes', [inputs]) as sc:
            ...
            conv4 = self.conv_conv_pool(up0, [128, 128], is_training, weight_decay, name="4", pool=False)
            conv5 = self.conv_conv_pool(conv4, [128, 128], is_training, weight_decay, name="5", pool=False)
            up1 = self.upconv_conc(conv5, conv1, 128, weight_decay, name="1", strides=3)
    ...
    def upconv_conc(self, inputA, input_B, n_filter, weight_decay, name, kernel_size=2, strides=2):
