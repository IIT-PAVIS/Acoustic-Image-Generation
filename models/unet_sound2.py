...
class UNetSound(object):
    ...
    def _build_network(self, inputs, is_training=True, keep_prob=0.5, weight_decay=8e-5, scope='UNetAudio'):
        """
        ...
        with tf.variable_scope(scope, 'UNetAudio', [inputs]) as sc:
            ...
            mean = tf.layers.conv2d(conv5, 256, (12, 16), padding='VALID', name='mean')
            mean = tf.reshape(mean, (-1, 256))
            std = tf.nn.softplus(tf.layers.conv2d(conv5, 256, (12, 16), padding='VALID', name='std'))
