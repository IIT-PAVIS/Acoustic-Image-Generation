...
class UNetAc(object):
    ...
    def _build_network(self, inputs, is_training=True, keep_prob=0.5, weight_decay=1e-6, scope='UNetAcoustic'):
        ...
        with tf.variable_scope(scope, 'UNetAcoustic', [inputs]) as sc:
            ...
            #net = X / 127.5 - 1
            conv1, pool1 = self.conv_conv_pool(inputs, [64, 64], is_training, weight_decay, name="1")
            conv2, pool2 = self.conv_conv_pool(pool1, [64, 64], is_training, weight_decay, name="2")
            conv3 = self.conv_conv_pool(
                pool2, [128, 128], is_training, weight_decay, name="3", pool=False)

            # embedding = math_ops.reduce_mean(conv5, [1, 2], keepdims=True)
            mean = tf.layers.conv2d(conv3, 128, (9, 12), padding='VALID', name='mean')
            mean = tf.reshape(mean, (-1, 128))
            variance = tf.layers.conv2d(conv3, 128, (9, 12), padding='VALID', name='variance')
            ...
            net = tf.layers.dense(guessed_z, 9 * 12, activation=tf.nn.relu)
            net = tf.reshape(net, (-1, 9, 12, 1))
            net = tf.layers.conv2d(net, 128, (3, 3), activation=tf.nn.relu, padding='same')

            up1 = self.upconv_concat(net, conv2, 64, weight_decay, name="1")
            conv4 = self.conv_conv_pool(up1, [64, 64], is_training, weight_decay, name="4", pool=False)
            # conv5 = self.conv_conv_pool(conv4, [16, 16], is_training, weight_decay, name="5", pool=False)
            up2 = self.upconv_concat(conv4, conv1, 64, weight_decay, name="2")
            conv6 = self.conv_conv_pool(up2, [64, 64], is_training, weight_decay, name="6", pool=False)
            # conv7 = self.conv_conv_pool(conv6, [2, 2], is_training, weight_decay, name="7", pool=False)
