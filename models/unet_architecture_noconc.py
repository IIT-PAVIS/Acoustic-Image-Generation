...
class UNet(object):
    ...
    def _build_network(self, inputs, is_training=True, keep_prob=0.5, weight_decay=7e-5, scope='UNet'):
        ...
        with tf.variable_scope(scope, 'UNet', [inputs]) as sc:
            ...
            #net = X / 127.5 - 1
            conv1, pool1 = self.conv_conv_pool(inputs, [32, 32], is_training, weight_decay, name="1", strides=(3, 3), padding='valid', filters=(3, 3))
            conv2, pool2 = self.conv_conv_pool(pool1, [128, 128], is_training, weight_decay, name="2", padding='valid', filters=(3, 3))
            conv3, pool3 = self.conv_conv_pool(pool2, [256, 256], is_training, weight_decay, name="3", strides=(3, 3), padding='valid', filters=(2, 3))
            ...
            mean = tf.layers.conv2d(conv5, 1024, (12, 16), padding='VALID', name='mean')
            mean = tf.reshape(mean, (-1, 1024))
            std = tf.nn.softplus(tf.layers.conv2d(conv5, 1024, (12, 16), padding='VALID', name='std'))
