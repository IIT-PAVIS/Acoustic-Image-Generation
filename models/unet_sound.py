...
class UNetSound(object):
    ...
    def _build_network(self, inputs, is_training=True, keep_prob=0.5, weight_decay=6e-5, scope='UNetAudio'):
        ...
        with tf.variable_scope(scope, 'UNetAudio', [inputs]) as sc:
            ...
            #net = X / 127.5 - 1
            conv1, pool1 = self.conv_conv_pool(inputs, [8, 8], is_training, weight_decay, name="1", padding='valid', filters=(3, 3))
            conv2, pool2 = self.conv_conv_pool(pool1, [8, 8], is_training, weight_decay, name="2", padding='valid', filters=(3, 2))
            conv3, pool3 = self.conv_conv_pool(pool2, [32, 32], is_training, weight_decay, name="3")
            conv4, pool4 = self.conv_conv_pool(pool3, [64, 64], is_training, weight_decay, name="4")
            ...
            mean = tf.layers.conv2d(conv5, 128, (6, 16), padding='VALID', name='mean')
            mean = tf.reshape(mean, (-1, 128))
            variance = tf.layers.conv2d(conv5, 128, (6, 16), padding='VALID', name='variance')
            ...
            net = tf.layers.dense(guessed_z, 6 * 16, activation=tf.nn.relu)
            net = tf.reshape(net, (-1, 6, 16, 1))
            net = tf.layers.conv2d(net, 128, (3, 3), activation=tf.nn.relu, padding='same')

            up6 = self.upconv_concat(net, conv4, 64, weight_decay, name="6")
            ...
            up8 = self.upconv_concat(conv7, conv2, 8, weight_decay, name="8", kernel_size=[3, 2])
            conv8 = self.conv_conv_pool(up8, [8, 8], is_training, weight_decay, name="8", pool=False)

            up9 = self.upconv_concat(conv8, conv1, 8, weight_decay, name="9", kernel_size=[3, 3])
